const ApiKey = "782dd14543c540f8af0dd15e6d45d192";
...
function getListTeams() {
    title.innerHTML = "<b>List Team Premiere League 2022</b>"
...
function showTeamInfo(id){
    fetch(deskripsi+id,fetchHeader)
    .then(response => response.json())
    .then(resJson => {
        console.log(resJson.competition);
        let competitions = "";
        let namatim = `<h4> ${resJson.name} </h4>`
        let logo = `<img src = "${resJson.crestUrl}">`
        let tahun = `<h6> ${resJson.founded} </h6>`
        let stadium = `<h6> ${resJson.venue} </h6>`
        let alamat = `<h6> ${resJson.address} </h6>`
        let email = `<h6> ${resJson.email} </h6>`
        let website = `<h6> ${resJson.website} </h6>`

        resJson.squad.forEach(liga=>{
            competitions +=`
            <h4>--------------------------------</h4>
            <h5>Nama = ${liga.name}</h5>
            <h5>Posisi = ${liga.position}</h5>
            <h5>Asal = ${liga.nationality}</h5>
            <h5>Tanggal Lahir = ${liga.dateOfBirth}</h5>
            `
        })
        contents.innerHTML=namatim + logo + tahun + stadium + alamat + email + website + "<h3><b>List Pemain</b></h3>"+ competitions;
    }).catch(err => {
        console.error(err);
    })
}
